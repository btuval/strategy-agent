...
const KPIStat = ({ label, value, target, variance, isPositiveGood = true, calculationLogic }) => (
  <CalculationTooltip calculationLogic={calculationLogic} className="block w-full">
    <div className="bg-[#0f172a]/70 rounded-xl p-4 border border-white/5 w-full">
      ...
      <div className="flex items-center gap-2">
        ...
        {variance && (
          <span className={`text-[10px] font-bold flex items-center gap-0.5 ${(isPositiveGood ? parseFloat(variance) < 0 : parseFloat(variance) > 0) ? 'text-red-400' : 'text-emerald-400'}`}>
            {(isPositiveGood ? parseFloat(variance) < 0 : parseFloat(variance) > 0) ? <TrendingDown className="w-3 h-3" /> : <TrendingUp className="w-3 h-3" />}
            {variance}
          </span>
        )}
      </div>
      ...
      {isExpanded && (
        <div className="border-t border-white/10 px-6 pb-6 pt-5 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <KPIStat label="Customers Acquired" value={customersActual?.toLocaleString()} target={targetCustomers?.toLocaleString()} variance={`${variancePct}%`} calculationLogic="Customers acquired = sum of new adds over campaign period. Variance % = (Actual − Target) / Target × 100." />
            <KPIStat label="Revenue" value={`$${(revenueActual / 1_000_000).toFixed(1)}M`} target={`$${(revenueTarget / 1_000_000).toFixed(2)}M`} variance={`${variancePct}%`} calculationLogic="Revenue = customers acquired × (ARPU or LTV contribution) over the period. Variance % vs target." />
            <KPIStat label="Cost Per Acquisition" value={`$${cpa}`} target={`$${cpTarget}`} variance={`+$${(cpa - cpTarget).toFixed(2)}`} isPositiveGood={false} calculationLogic="CPA = Total campaign spend / Customers acquired. Lower is better." />
            <KPIStat label="Conversion Rate" value={`${conversionRate}%`} target={`${conversionTarget}%`} variance={`${(conversionRate - conversionTarget).toFixed(1)}pp`} calculationLogic="Conversion rate = (Conversions / Leads or impressions) × 100. pp = percentage points vs target." />
            <KPIStat label="Early Churn (New Cohort)" value={`${earlyChurn}%`} target={`${churnTarget}%`} variance={`+${(earlyChurn - churnTarget).toFixed(1)}pp`} isPositiveGood={false} calculationLogic="Early churn % = (Churned in cohort / Cohort size) × 100 within the measurement window." />
          </div>
